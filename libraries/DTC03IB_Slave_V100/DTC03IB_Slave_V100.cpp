...
#if ENABLE_REMOTE_MODE //sherry++ 2017.9.20
    case I2C_COM_REMOTE:
      //Serial.print(bRemoteMode);
...
#if 0 //ENABLE_REMOTE_MODE //sherry++ 2017.10.3
    case I2C_COM_RVSET:
      if (bRemoteMode)
      {
        temp[0] = g_vset_limit;
...
#if ENABLE_REMOTE_MODE //sherry** 2017.9.25
void DTC03::CheckSerial()
{
  ...
  {
    //t1 = micros();
    //sInputString = Serial.readString();
    sInputString = Serial.readStringUntil('\n');
    //t2 = micros();
    ucInputNum = sInputString.length();
    sInputString.toCharArray(cInputStr, ucInputNum+1);
    cInputStr[ucInputNum] = '\0';
    //Serial.print(cInputStr);
    ...
    if (ucInputNum > 0)
    {
      if (strcmp(cInputStr, SERIAL_CMD_IDN) == 0)
      {
        //sInputString += REMOTE_IDN;
        //Serial.print(sInputString);
        Serial.println(REMOTE_IDN_TEXT);
      }
      else if (strcmp(cInputStr, SERIAL_CMD_REM) == 0)
      {
        bRemoteMode = true;
        //sInputString += bRemoteMode;
        //sInputString += " OK"; 
        //Serial.print(sInputString);
      }
      else if (bRemoteMode)
      {
        if (strncmp(cInputStr, SERIAL_CMD_TSET, 5) == 0)
        {
          String sTempStr = sInputString.substring(5);
          long ucTemp = sTempStr.toInt();
          if (sTempStr.toInt() > 65535)
            ; //Serial.print("TOO LOW");
          else if (ucTemp > REMOTE_TSET_MAX)
            ; //Serial.print("TOO LOW");
          else if (ucTemp < REMOTE_TSET_MIN)
            ; //Serial.print("TOO HIGH");
          else
            g_vset_limit = ucTemp;
          setVset();
          //Serial.println(ucTemp);
          //sInputString += " OK"; 
        }
        else if (strcmp(cInputStr, SERIAL_CMD_TACT) == 0)
        {
        ...
        {
          g_en_state = 1;
          //sInputString += g_en_state;
          //sInputString += " OK"; 
        }
        else if (strcmp(cInputStr, SERIAL_CMD_STOP) == 0)
        {
          g_en_state = 0;
          //sInputString += g_en_state;
          //sInputString += " OK"; 
        }
        else if (strcmp(cInputStr, SERIAL_CMD_UREM) == 0)
        {
          bRemoteMode = false;
          //sInputString += bRemoteMode;
          //sInputString += " OK"; 
...
//sherry++ 2017.9.25
void DTC03::SerialInit(bool bFromSetup)
{
  if (bFromSetup)
  {
    bRemoteMode = false;    
    //sherry++ 2017.9.29 stop
    g_en_state = 0;
  }
  ucInputNum = 0;
  // reserve 200 bytes for the inputString:
  sInputString.reserve(SERIAL_MAX_INPUT_NUM);
  // clear the string:
  sInputString = "";
  cInputStr[0] = '\0';
}
