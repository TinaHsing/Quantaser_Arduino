...
const unsigned char CRC_8_CCITT_Table[] = {
...
void DTC03Master::CheckStatus()
{
    ...
    if (p_loopindex % 300 == 2)
    {
        ...
        if (p_engModeFlag && (g_cursorstate > 9))
            PrintTpcb(tpcb_f);
    }
...
void DTC03Master::I2CWriteData(unsigned char com)
{
    ...
    case I2C_COM_INIT:
        ...
        if (g_mod_status)
            temp[1] |= REQMSK_SENSTYPE; //B01000000
        break;
...
void DTC03Master::UpdateParam() // Still need to add the upper and lower limit of each variable
{
    ...
    if (g_paramupdate)
    {
        ...
        case 10:
            g_r1 += g_counter;
            if (g_r1 > 30)
                g_r1 = 30; // R1, 1~30 for 0.1~3.0 ohm set
            if (g_r1 < 1)
                g_r1 = 1; //
        ...
        case 11:
            ...
            if (g_r2 > 30)
                g_r2 = 30;
            if (g_r2 < 1)
                g_r2 = 1; //R2, 1~30 for 1.0~3.0 ohm set
            I2CWriteData(I2C_COM_R1R2);
            PrintR2();
        ...
        case 12:
            g_tpidoff += g_counter;
            if (g_tpidoff > 10)
                g_tpidoff = 10; //Tpid offset, 0~10 for 1~10000 @1000 step
            if (g_tpidoff < 1)
                g_tpidoff = 1;
        ...
        case 19:
            ...
            if (g_stableCode_atune > 99)
                g_stableCode_atune = 99;
            if (g_stableCode_atune < 1)
                g_stableCode_atune = 1;
