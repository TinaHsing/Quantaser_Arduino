...
void DTC03Master::I2CWriteData(unsigned char com)
{
  ...
  {
    case I2C_COM_INIT:
        temp[0]= (unsigned int)g_bconst - BCONSTOFFSET;
        temp[1]= ((unsigned int)g_bconst - BCONSTOFFSET) >> 8;
...
{
  unsigned char temp[2], b_upper, b_lower, vset_upper, vset_lower;
...
#if ENABLE_REMOTE_MODE //sherry++ 2017.9.20
    case I2C_COM_REMOTE:
      bOrignalRemote = CheckSlaveRemote();

      if (temp[0] == 1)
      {
        SetSlaveRemote(true);
        g_LCDlock_flag = true;
        if (bOrignalRemote == false)
        {
          lcd.ClearScreen(0);
          PrintRemoteText();
        }
      ...
      {
        SetSlaveRemote(false);
        g_LCDlock_flag = false;
        if (bOrignalRemote == true)
        {
          lcd.ClearScreen(0);
          BackGroundPrint();
          PrintNormalAll();
...
#if ENABLE_REMOTE_MODE //sherry++ 2017.9.19
void DTC03Master::PrintRemoteText()
{
  lcd.SelectFont(Iain5x7);
  lcd.GotoXY(TSET_COORD_X,TSET_COORD_Y);
  lcd.print(Text_SET);
...
//sherry++ 2017.9.19
bool DTC03Master::CheckSlaveRemote()
{
  return g_bSlaveRemote;
}

void DTC03Master::SetSlaveRemote(bool bSlaveRemote)
{
   g_bSlaveRemote = bSlaveRemote;
}
